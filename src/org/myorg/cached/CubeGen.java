...
public class CubeGen extends Configured implements Tool {
	...
	public int run(String[] args) throws Exception {
		...
		runGenJob("CubeCached_"+jobName+"_1", pOriginInput, pOriginOutput, true, isDistributive);
		
		// Generate the new cube
		Path pExtraInput = new Path(args[3]);
		Path pExtraOutput = new Path(args[4]);
		runGenJob("CubeCached_"+jobName+"_2", pExtraInput, pExtraOutput, false, isDistributive);
